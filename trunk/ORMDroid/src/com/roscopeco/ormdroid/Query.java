...
public class Query<T extends Entity> {
  ...
  public Query<T> and(Object value) {
    mSb.append(" AND ").append(value);
    return this;
  }
  ...
  public Query<T> in(String query) {
	    mSb.append(" in ").append("(").append(query).append(")");
	    return this;
	  }
  public Query<T> isNotNull() {
	    mSb.append(" IS NOT NULL ");
	    return this;
	  }
  ...
  public List<T> executeMultiDesc() {
	    SQLiteDatabase db = ORMDroidApplication.getDefaultDatabase();
	    try {
	      return executeMultiDesc(db);
	    } finally {
	      db.close();
	    }
	  }
  public List<T> executeHealthAssertDesc() {
	    SQLiteDatabase db = ORMDroidApplication.getDefaultDatabase();
	    try {
	      return executeHealthAssertDesc(db);
	    } finally {
	      db.close();
	    }
	  }
  
  public List<T> executeLastExerCise() {
	    SQLiteDatabase db = ORMDroidApplication.getDefaultDatabase();
	    try {
	      return executeLastExerCise(db);
	    } finally {
	      db.close();
	    }
	  }
  public List<T> executeLastHealtAssert() {
	    SQLiteDatabase db = ORMDroidApplication.getDefaultDatabase();
	    try {
	      return executeLastHealtAssert(db);
	    } finally {
	      db.close();
	    }
	  }
  ...
  public List<T> executeMultiDesc(SQLiteDatabase db) {
	    String sql = mSb.toString()+ " order by id desc";
	    Log.v(TAG, sql);
	    return Entity.getEntityMappingEnsureSchema(db, mClass).loadAll(db, db.rawQuery(mSb.toString(), null));
	  }
  public List<T> executeHealthAssertDesc(SQLiteDatabase db) {
	    String sql = mSb.toString()+ " order by assertDate asc";
	    Log.v(TAG, sql);
	    return Entity.getEntityMappingEnsureSchema(db, mClass).loadAll(db, db.rawQuery(mSb.toString(), null));
	  }
  
  public List<T> executeLastExerCise(SQLiteDatabase db) {
	    String sql = "SELECT *  FROM comaplicationbooksportmodelExercise order by id desc LIMIT 0, 7";
	    Log.v(TAG, sql);
	    return Entity.getEntityMappingEnsureSchema(db, mClass).loadAll(db, db.rawQuery(sql, null));
	  }
  public List<T> executeLastHealtAssert(SQLiteDatabase db) {
	    String sql = "SELECT *  FROM comaplicationbooksportmodelHealthAssert order by id desc LIMIT 0, 1";
	    Log.v(TAG, sql);
	    return Entity.getEntityMappingEnsureSchema(db, mClass).loadAll(db, db.rawQuery(sql, null));
	  }
}
